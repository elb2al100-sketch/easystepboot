...
const jeda  = 15; // 15 seconds delay / 15 ثوانٍ تأخير
...
// Fetch group info using external API / جلب معلومات المجموعة عبر API خارجي
...
async function handle(sock, messageInfo) {
    ...
    const useMentions = false; // Set true if you want to mention all participants / ضع true إذا أردت ذكر جميع المشاركين
    ...
    try {
        if(isRunning) {
            return await sock.sendMessage(
                remoteJid,
                { text: `⚠️ _JPM process is already running. Please wait until it finishes._` },
                { quoted: message }
            );
        }
        ...
        if (groupIds.length === 0) {
            ...
            return await sock.sendMessage(
                remoteJid,
                { text: `⚠️ No groups matching the condition found.` },
                { quoted: message }
            );
        }
...
function sendErrorMessage(sock, remoteJid, message, prefix, command) {
    return sock.sendMessage(
        remoteJid,
        {
            text: `_⚠️ Usage Format:_ \n\n_💬 Example:_ _*${prefix + command} WhatsApp bot announcement*_`
            // _⚠️ صيغة الاستخدام:_
            // مثال: .jpm إعلان بوت واتساب
        },
        { quoted: message }
    );
}
