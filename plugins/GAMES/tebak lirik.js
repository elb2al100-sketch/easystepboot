...
const { addUser, removeUser, getUser, isUserPlaying } = require("@tmpDB/tebak lirik"); // Temporary DB / قاعدة بيانات مؤقتة

// ===== MAIN HANDLE FUNCTION / الدالة الرئيسية =====
async function handle(sock, messageInfo) {
    const { remoteJid, message, fullText } = messageInfo;
    
    // Skip if message does not contain 'lirik' / تخطي إذا لم تحتوي الرسالة على 'lirik'
    if (!fullText.includes("lirik")) {
        return true;
    }

    try {
        // Call API to get lyrics question / استدعاء API للحصول على جزء من كلمات الأغنية
        ...
        // Send the lyrics question to user / إرسال السؤال للمستخدم
        await sock.sendMessage(
            remoteJid,
            { text: `Please answer the following / الرجاء الإجابة على السؤال التالي\n\n${soal}\nTime / الوقت : ${WAKTU_GAMES}s` },
            { quoted: message }
        );

        // Log the correct answer / تسجيل الإجابة الصحيحة
        logWithTime('Tebak Lirik / تخمين كلمات الأغنية', `Answer / الإجابة : ${jawaban}`);
...
module.exports = {
    handle,
    Commands: ["tebak", "tebaklirik"], // Commands / الأوامر
    OnlyPremium: false,
    OnlyOwner: false,
};