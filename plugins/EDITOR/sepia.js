...
/**
 * Handle the sepia command
 * معالجة أمر تحويل الصورة إلى تأثير سبيّا (Sepia)
 */
async function handle(sock, messageInfo) {
    ...
    try {
        ...
        await sharp(mediaPath)
            .modulate({ saturation: 0.5 }) // Reduce saturation / تقليل التشبع
            .tint({ r: 112, g: 66, b: 20 }) // Apply sepia tint / تطبيق لون السبيّا
        ...
        if (fs.existsSync(outputImagePath)) {
            ...
            throw new Error("Sepia file not found.");
        }
    } catch (error) {
